...
**3827 Edwards** and **3829 Edwards** (status Under Construction) - built from two Amna Design finish-selection decks (`tan-development projects/Amna Design - Tan Development - 3827.pdf` and `...3829.pdf`), not from real estate listings. Real facts used: the exact address (stated in both documents), exterior materials, and interior finish specs (paint colors, stone, fixtures - see each PDF for the full spec sheet). Three things are **not** in either document and need to be confirmed against the actual plans:
1. **Square footage** - not stated anywhere in either deck (no floor plans, no dimensions). I used placeholder estimates (2,400 and 2,350 sqft) based on the similar room program to 3819 Edwards. Replace with the real number.
2. **Bed/bath count** - I inferred 4 bed / 4.5 bath for both from the room list (1 primary + 3 guest baths + 1 powder in each deck), but this is an inference, not a stated fact. Confirm against the actual plans.
3. **Neighborhood** - neither document names a neighborhood. I assumed "Inglewood" since both addresses are on the same short stretch of Edwards Ave as the confirmed-Inglewood 3819 listing. Very likely correct, but not explicitly confirmed.
4. The gallery images on both project pages are **photorealistic renderings from the design deck, not construction photos** - the pages already carry a visible "Renderings shown" note (see `src/pages/portfolio/[slug].astro`) since both stages are pre-construction, but replace them with real photos once each home is built.
